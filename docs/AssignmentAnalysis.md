...
## Overview
-  Using both filters (Amsterdam and Amsterdam with garden), the dataset sizes are relatively small (around 6k records in total).
-  Assuming the dataset size does not grow significantly, it should be possible to fetch all records without hitting the API rate limit (100 req/min) by increasing the page size.
-  Another factor is feed frequency, which is expected to be around once a day (perhaps slightly more often).

## Proposed Approach (Keep It Simple)
-  Run a process that eagerly queries the API to compute the result for both filters.
-  Store the computed result per filter (cache or DB).
-  Refresh the data fully based on how frequently new data appears (assumed 1-2 times per day).
-  Expose an endpoint for clients that returns the stored result.
